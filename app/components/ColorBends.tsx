...
void main() {
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    // Adjusted exponential factors for more vivid/sharp colors
    float expFactor = 4.0 * uContrast; // Lower = sharper bands
    float expDivisor = 8.0 * uBrightness; // Higher = brighter overall
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            // Sharper, more vivid color bands
            float w = 1.0 - exp(-expDivisor / exp(expFactor * m));
            // Boost intensity
            w = pow(w, 0.7);
            sumCol += uColors[i] * w;
            cover = max(cover, w);
      }
      // Allow colors to go slightly beyond 1.0 for HDR-like effect, then clamp
      col = clamp(sumCol * 1.3, 0.0, 1.0);
      a = uTransparent > 0 ? clamp(cover * 1.2, 0.0, 1.0) : 1.0;
    } else {
        vec2 s = q;
        for (int k = 0; k < 3; ++k) {
            s -= 0.01;
